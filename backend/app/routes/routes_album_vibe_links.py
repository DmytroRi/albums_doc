...
def _get_record(session: Session, album_id: int, vibe_id: int):
    return session.exec(
        select(AlbumVibeLink).where(
            AlbumVibeLink.album_id == album_id,
            AlbumVibeLink.vibe_id == vibe_id,
        )
    ).first()
...
def create_album_vibe_link(
    payload: AlbumVibeLinkCreate,
    session: Session = Depends(get_session),
):
    try:
...
def get_album_vibe_link(
    ...
    try:
        ...
        if not record:
            raise HTTPException(
                status_code=404,
                detail="AlbumVibeLink not found",
            )
        return record
    except HTTPException:
        raise
