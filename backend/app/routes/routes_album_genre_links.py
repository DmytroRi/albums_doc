...
def _get_record(session: Session, album_id: int, genre_id: int):
    return session.exec(
        select(AlbumGenreLink).where(AlbumGenreLink.album_id == album_id, AlbumGenreLink.genre_id == genre_id)
    ).first()
...
def create_album_genre_link(payload: AlbumGenreLinkCreate, session: Session = Depends(get_session)):
    try:
...
def get_album_genre_link(album_id: int, genre_id: int, session: Session = Depends(get_session)):
    try:
        record = _get_record(session, album_id, genre_id)
        if not record:
            raise HTTPException(status_code=404, detail="AlbumGenreLink not found")
        return record
    except HTTPException:
        raise
    except (IntegrityError, SQLAlchemyError) as exc:
...
def update_album_genre_link(
    album_id: int, genre_id: int, payload: AlbumGenreLinkUpdate, session: Session = Depends(get_session)
):
    try:
        record = _get_record(session, album_id, genre_id)
        if not record:
...
def delete_album_genre_link(album_id: int, genre_id: int, session: Session = Depends(get_session)):
    try:
        record = _get_record(session, album_id, genre_id)
        if not record:
            raise HTTPException(status_code=404, detail="AlbumGenreLink not found")
